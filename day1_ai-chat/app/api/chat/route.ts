...
export async function POST(req: Request) {
  const { message, sessionId, model, files, strategy, windowSize, profileId, invariants } = await req.json();
  ...
  try {
    const stream = agent.chat(message, files, profileId ? Number(profileId) : undefined, invariants as string[] | undefined);
